...
public class request extends JFrame implements ActionListener{
	...
	panel.setBorder(new TitledBorder(new LineBorder(new Color(47, 79, 79), 2, true), "Request-Book",
    ...
    public void actionPerformed(ActionEvent ae){
		...
		st.setString(1, t15.getText());
		ResultSet rs = st.executeQuery();
		
                while (rs.next()) {
                    t2.setText(rs.getString("label_id"));
		...
		rs.close();
            ...
            if(ae.getSource() == b3){
                ...
                if(Integer.parseInt(t20.getText()) == 0) {

                            String sql = "insert into request(label_id, student_id, bname, author, sname, course, branch, dateOfRequest) values(?, ?, ?, ?, ?, ?, ?, ?)";
                            PreparedStatement st = con.c.prepareStatement(sql);
